<?php
	...
	if(isset($_REQUEST['show_invoice_id'])) { 
        $show_invoice_id = filter_input(INPUT_GET, 'show_invoice_id', FILTER_SANITIZE_SPECIAL_CHARS);

        $show_invoice_id = trim($show_invoice_id);
        ...
        $current_date = date("Y-m-d"); $bank_id = "";
        $invoice_date = date('Y-m-d'); $bill_number = ""; $party_id = ""; $product_count = 0; $gst_option = 1; $tax_option = 1;$tax_type = "";$party_state = ""; $company_state = "";
        $company_state = $obj->getTableColumnValue($GLOBALS['company_table'], 'company_id', $GLOBALS['bill_company_id'], 'state');
        $product_tax = array(); $product_count = 0; $estimate_number = "";
        ...
        if(!empty($show_invoice_id)) {
            ...
            if(!empty($conversion_update) && $conversion_update == '1') {
                $invoice_list = $obj->getTableRecords($GLOBALS['estimate_table'], 'estimate_id', $show_invoice_id,'');
            }else{
                $invoice_list = $obj->getTableRecords($GLOBALS['invoice_table'], 'invoice_id', $show_invoice_id,'');
            }
                ...
                foreach($invoice_list as $data) {
                    if(!empty($conversion_update) && $conversion_update == '1') {
                        ...
                        if(!empty($data['estimate_date'])) {
                            $invoice_date = date('Y-m-d', strtotime($data['estimate_date']));
                        }
                    }
        ...
        <form class="poppins pd-20 redirection_form" name="invoice_form" method="POST">
			<div class="card-header">
				<div class="row p-2">
                    <div class="col-lg-8 col-md-8 col-8 align-self-center">
                        <?php if(empty($show_invoice_id) || !empty($conversion_update)) { ?>
             ...
             <?php if(!empty($estimate_number) && $estimate_number != $GLOBALS['null_value']){  ?>
                <div class="row justify-content-center p-3" >
                    <div class="col-lg-12 col-md-4 col-6 py-2 px-lg-1 text-center" style ="font-weight : bold;font-size: 20px;">
                    <?php echo 'Estimate No : ' .$estimate_number;  ?>
                    </div>
                </div>
            <?php } ?>
            <div class="row justify-content-center p-3">

                <input type="hidden" name="edit_id" value="<?php if(!empty($show_invoice_id) && empty($conversion_update)) { echo $show_invoice_id; } ?>">
                <?php if($conversion_update == '1') { ?>
                    <input type="hidden" name="conversion_update" value="1">
                    <input type="hidden" name="conversion_id" value="<?php if(!empty($show_invoice_id)) { echo $show_invoice_id; } ?>">
                <?php } ?> 
                <div class="col-lg-2 col-md-4 col-6 py-2 px-lg-1">
                    <div class="form-group">
                        <div class="form-label-group in-border">
                            <input type="date" id="invoice_date" name="invoice_date" class="form-control shadow-none" placeholder="" value="<?php if(!empty($invoice_date)){ echo $invoice_date; }?>" max="<?php if(!empty($current_date)){ echo $current_date; }?>">
                ...
                <div class="col-lg-3 col-md-4 col-6 py-2 px-lg-1">
                    <div class="form-group">
                        <div class="form-label-group in-border">
                            <div class="input-group  flex-nowrap" <?php if(!empty($conversion_update) && $conversion_update == '1' && (!empty($show_invoice_id))) { ?> style="pointer-events:none;" <?php } ?>>
                                <select class="select2 select2-danger" name ="party_id" data-dropdown-css-class="select2-danger" style="width: 100%;" onchange="Javascript:getPartyState(this.value);" <?php if(!empty($conversion_update) && $conversion_update == '1' && (!empty($show_invoice_id))) { ?> tabindex = -1 <?php } ?>>
                ...
                <div class="col-lg-2 col-md-4 col-6 px-lg-1 py-2 d-none">
                    <div class="form-group mb-1">
                        <div class="flex-shrink-0">
                            <div class="form-check form-switch form-switch-right form-switch-md">
                                <label for="gst_option" class="form-label text-muted smallfnt">GST/Non GST</label>
                                <input class="form-check-input code-switcher" type="checkbox" name="gst_option" checked onchange="Javascript:GetTaxType(this,this.value);"
                            ...
                            <select class="select2 select2-danger"  name="bank_id" data-dropdown-css-class="select2-danger" style="width: 100%;">
                                ...
                                <?php if(!empty($bank_list)) {
                                    foreach($bank_list as $data) { ?>
                                        <option value="<?php if(!empty($data['bank_id'])) { echo $data['bank_id']; } ?>" <?php if(!empty($data['bank_id']) && ($data['bank_id'] == $bank_id))  { ?>selected<?php } ?>><?php if(!empty($data['bank_name'])) { echo $obj->encode_decode('decrypt', $data['bank_name']); } ?></option>
                                <?php }
                                } ?>
                            </select>
            ...
            <div class="row justify-content-center">    
                ...
                <div class="col-md-12 py-3 text-center">
                    <button class="btn btn-danger submit_button" type="button" onclick="Javascript:SaveModalContent(event, 'invoice_form', 'invoice_changes.php', 'invoice.php');">
                        Submit
                    </button>
                </div>
            </div>
      ...
      if(isset($_POST['edit_id'])) {
        ...
        if(isset($_POST['invoice_date'])) {
            ...
            $invoice_date_error = $valid->valid_date(date('Y-m-d', strtotime($invoice_date)), 'Bill Date', '1');
        ...
        if(isset($_POST['bank_id'])) {
            ...
            if(!empty($bank_id_error)) {
                ...
                $party_state = $obj->getTableColumnValue($GLOBALS['bank_table'], 'bank_id', $bank_id, 'state');
            }
        }
            ...
            if(!empty($notes)){
              $notes_error = $valid->common_validation($notes, 'Notes', 'select');
                if(!empty($notes_error)) {
                    if(!empty($valid_invoice)) {
                        $valid_invoice = $valid_invoice." ".$valid->error_display($form_name, 'notes', $notes_error, 'select');
                    }
                    else {
                        $valid_invoice = $valid->error_display($form_name, 'notes', $notes_error, 'select');
                    }
                }
                else {
        ...
        if(!empty($gst_option_error)) {
            if(!empty($valid_invoice)) {
                $valid_invoice = $valid_invoice." ".$valid->error_display($form_name, 'gst_option', $gst_option_error, 'input');
            }
            else {
                $valid_invoice = $valid->error_display($form_name, 'gst_option', $gst_option_error, 'input');
            }
        }
        ...
        if($gst_option == '1') {
            ...
            if($tax_type == '2') {
                ...
                if(!empty($overall_tax_error)) {
                    if(!empty($valid_invoice)) {
                        $valid_invoice = $valid_invoice." ".$valid->error_display($form_name, 'overall_tax', $overall_tax_error, 'select');
                    }
                    else {
                        $valid_invoice = $valid->error_display($form_name, 'overall_tax', $overall_tax_error, 'select');
                    }
                }
            }
        }
            ...
            for($i=0; $i < count($product_ids); $i++) {
                    ...
                    if(empty($quantity_error)){
                        ...
                        if(isset($rates[$i])) {
                            ...
                            if(empty($price_error)){
                                ...
                                if(!empty($valid_invoice)) {
                                    $valid_invoice = $valid_invoice." ".$valid->row_error_display($form_name, 'rates[]', $price_error, 'text', 'product_row', ($i+1));
                                }
                                else {
                                    $valid_invoice = $valid->row_error_display($form_name, 'rates[]', $price_error, 'text', 'product_row', ($i+1));
                                }
                            }
                        ...
                        if(!empty($valid_invoice)) {
                            $valid_invoice = $valid_invoice." ".$valid->row_error_display($form_name, 'quantity[]', $quantity_error, 'text', 'product_row', ($i+1));
                        }
                        else {
                            $valid_invoice = $valid->row_error_display($form_name, 'quantity[]', $quantity_error, 'text', 'product_row', ($i+1));
                        }
                    }
                ...
                if(isset($hsn_code[$i])) {
                    ...
                    if(!empty($hsn_error)){
                        ...
                        if(!empty($valid_invoice)) {
                            $valid_invoice = $valid_invoice." ".$valid->row_error_display($form_name, 'hsn_code[]', $hsn_error, 'text', 'product_row', ($i+1));
                        }
                        else {
                            $valid_invoice = $valid->row_error_display($form_name, 'hsn_code[]', $hsn_error, 'text', 'product_row', ($i+1));
                        }
                    }
                ...
                if(isset($description[$i])) {
                    ...
                    if(empty($description_error)){
                        ...
                        if(!empty($description[$i]) && !empty($description_error)){
                            ...
                            if(!empty($valid_invoice)) {
                                $valid_invoice = $valid_invoice." ".$valid->row_error_display($form_name, 'description[]', $description_error, 'text', 'product_row', ($i+1));
                            }
                            else {
                                $valid_invoice = $valid->row_error_display($form_name, 'description[]', $description_error, 'text', 'product_row', ($i+1));
                            }
                        }
                    }
        ...
        if(!empty($discount_name_error)) {
            if(!empty($valid_invoice)) {
                $valid_invoice = $valid_invoice." ".$valid->error_display($form_name, 'discount_name', $discount_name_error, 'text');
            }
            else {
                $valid_invoice = $valid->error_display($form_name, 'discount_name', $discount_name_error, 'text');
            }
        }
        if(!empty($discount)) {
        ...
        if(!empty($discount_error)) {
            if(!empty($valid_invoice)) {
                $valid_invoice = $valid_invoice." ".$valid->error_display($form_name, 'discount', $discount_error, 'text');
            }
            else {
                $valid_invoice = $valid->error_display($form_name, 'discount', $discount_error, 'text');
            }
        }
        ...
        if(!empty($extra_charges_name_error)) {
            if(!empty($valid_invoice)) {
                $valid_invoice = $valid_invoice." ".$valid->error_display($form_name, 'extra_charges_name', $extra_charges_name_error, 'text');
            }
            else {
                $valid_invoice = $valid->error_display($form_name, 'extra_charges_name', $extra_charges_name_error, 'text');
            }
        }
        if(!empty($extra_charges)) {
        ...
        if(!empty($extra_charges_error)) {
            if(!empty($valid_invoice)) {
                $valid_invoice = $valid_invoice." ".$valid->error_display($form_name, 'extra_charges', $extra_charges_error, 'text');
            }
            else {
                $valid_invoice = $valid->error_display($form_name, 'extra_charges', $extra_charges_error, 'text');
            }
        }
        $extra_charges_total = $total_amount;

        $greater_tax = ""; $str_tax = ""; $extra_charges_tax = 0; $product_tax_value = array();
        if($gst_option == '1' && empty($invoice_error) && empty($valid_invoice)) {
            ...
            if($tax_type =='1'){
                ...
                if(!empty($overall_tax)) {
                    ...
                    if(preg_match("/^[0-9]+(\\.[0-9]+)?$/", $tax)) {
                        ...
                        $invoice_error = "Invalid Overall tax";
                    }
                    if(!empty($product_ids) && empty($invoice_error) && empty($valid_invoice)) {
                        for($j=0; $j < count($product_ids); $j++) {
        ...
        if(empty($invoice_error) && empty($valid_invoice) && $total_amount <= '0') {
            $invoice_error = "Bill value cannot be 0";
        }
                if(!empty($edit_id) && empty($invoice_error) && empty($valid_invoice)) {
                    if(!empty($product_ids)) {
                         for($j=0; $j < count($product_ids); $j++) {
        ...
        if(empty($invoice_error) && empty($valid_invoice)){
            ...
            if(preg_match("/^\d+$/", $check_user_id_ip_address)) {
                ...
                if(!empty($conversion_id) && $conversion_update == '1') {
                    $estimate_id = $conversion_id;
                }
                else {
                    $estimate_id = $GLOBALS['null_value'];
                }
                ...
                if(empty($edit_id)){
                    ...
                    if(preg_match("/^\d+$/", $invoice_insert_id)) {
                        ...
                        $invoice_number = "";
                        $invoice_number = $obj->getTableColumnValue($GLOBALS['invoice_table'],'invoice_id',$invoice_id,'invoice_number');

                        $result = array('number' => '1', 'msg' => 'Invoice Successfully Created','redirection_page' =>'invoice.php','invoice_id' => $invoice_id);
                        	if(!empty($conversion_update) && $conversion_update == '1') { 

                                if(!empty($estimate_id)){
                                    $stock_unique_ids= array();
                                    $stock_unique_ids = $obj->getTableRecords($GLOBALS['stock_table'], 'bill_unique_id', $estimate_id,'');
                                    if(!empty($stock_unique_ids)) {
                                        foreach($stock_unique_ids as $val) {
                                            $stock_unique_id = "";
                                            if(!empty($val['id']) && $val['id'] != $GLOBALS['null_value']) {
                                                $stock_unique_id = $val['id'];
                                            }
                                            if(preg_match("/^\d+$/", $stock_unique_id)) {
                                                ...
                                                $columns = array('deleted');
                                                $values = array(1);
                                                $msg = $obj->UpdateSQL($GLOBALS['stock_table'], $stock_unique_id, $columns, $values, $action);
								...
								$estimate_unique_id = "";
								$estimate_unique_id = $obj->getTableColumnValue($GLOBALS['estimate_table'], 'estimate_id', $estimate_id, 'id');
							
								if(preg_match("/^\d+$/", $estimate_unique_id)) {
									$action = "";
									$columns = array(); $values = array();			
									$columns = array('converted','conversion_number','conversion_id');
									$values = array(1,$invoice_number,$invoice_id);
                ...
                else
                {
                    ...
                    if(preg_match("/^\d+$/", $invoice_update_id)) {
                        $invoice_id = $edit_id; $update_stock = 1; $balance = 1;
                        $invoice_number = $obj->getTableColumnValue($GLOBALS['invoice_table'], 'invoice_id', $invoice_id, 'invoice_number');
                ...
                // if($update_stock == '1' && !empty($invoice_id) && !empty($invoice_number)) {
                //     $remarks = $invoice_number;
                ...
                //     if(!empty($product_ids)) {
                //         for($i=0; $i < count($product_ids); $i++) {
                //             $stock_update = $obj->StockUpdate($GLOBALS['invoice_table'], 'Out', $invoice_id, $invoice_number, $product_ids[$i], $unit_ids[$i],$size_ids[$i], $remarks, $invoice_date, $quantity[$i],$party_id); 
                //         }
                //     }
                // }
    ...
    if(isset($_POST['draw'])){
        ...
        $totalRecords = 0;
        $totalRecords = count($obj->getInvoiceRecordList($row, $rowperpage, $searchValue, $filter_from_date, $filter_to_date, $filter_party_id, $cancelled, $order_column, $order_direction));
        $filteredRecords = count($obj->getInvoiceRecordList('', '', $searchValue, $filter_from_date, $filter_to_date, $filter_party_id, $cancelled, $order_column, $order_direction));

        $data = [];
        
        $invoiceList = $obj->getInvoiceRecordList($row, $rowperpage, $searchValue, $filter_from_date, $filter_to_date, $filter_party_id, $cancelled, $order_column, $order_direction);
        $sno = $row + 1;
        foreach ($invoiceList as $val) {
            $created_date_time = $by_creator_name = ""; $updated_date_time = ""; $invoice_date = ""; $bill_number = ""; $party_name = "";$party_mobile_number =  $bill_total = 0;$invoice_number =""; $party_name_mobile_city ='';$product_count =0 ;
            ...
            if(!empty($val['invoice_date']) && $val['invoice_date'] != "0000-00-00") {
            ...
            if(!empty($val['invoice_number']) && $val['invoice_number'] != $GLOBALS['null_value']) {
            ...
            $edit_access_error = ""; $permission_module = $GLOBALS['invoice_module']; $permission_action = $edit_action;
            if(!empty($login_role_id)) {
                include("user_permission_action.php");
            }
            $delete_access_error = ""; $permission_module = $GLOBALS['invoice_module']; $permission_action = $delete_action;
            if(!empty($login_role_id)) {
                include("user_permission_action.php");
            }
            if(empty($edit_access_error) && empty($val['cancelled'])) {
                $edit_option = '<li><a class="dropdown-item" href="Javascript:ShowModalContent('.'\''.$page_title.'\''.', '.'\''.$val['invoice_id'].'\''.');"><i class="fa fa-pencil"></i>&nbsp; Edit</a></li>';
            }
            if(empty($delete_access_error) && empty($val['cancelled'])) {
                $delete_option = '<li><a class="dropdown-item" href="Javascript:DeleteModalContent('.'\''.$page_title.'\''.', '.'\''.$val['invoice_id'].'\''.');"><i class="fa fa-ban"></i>&nbsp; Cancel</a></li>';
            }
            $conversion_option = "";
            $print_option = '<li><a class="dropdown-item" target="_blank" href="reports/rpt_invoice_a4.php?view_invoice_id=' . $val['invoice_id'] . '"><i class="fa fa-print"></i>&nbsp; A4 Print</a></li>';

             if(empty($edit_access_error) && empty($val['cancelled'])) {
                // $conversion_option = '<li><a class="dropdown-item" href="Javascript:ShowModalContent('.'\''.$page_title.'\''.', '.'\''.$val['invoice_id'].'\''.');"><i class="bi bi-arrow-left-right"></i>&nbsp; Conversion</a></li>';
            }
            $action = '<div class="dropdown">
                            <a href="#" role="button" class="btn btn-dark py-1 px-2" id="dropdownMenuLink'.$val['invoice_id'].'" data-bs-toggle="dropdown" aria-expanded="false">
                                <i class="bi bi-three-dots-vertical"></i>
                            </a>
                            <ul class="dropdown-menu" aria-labelledby="dropdownMenuLink'.$val['invoice_id'].'">
                                '.$print_option.$edit_option.$conversion_option.$delete_option.'
                            </ul>
                        </div>';
    ...
    if(isset($_REQUEST['delete_invoice_id'])) {
        $delete_invoice_id = trim(filter_input(INPUT_GET, 'delete_invoice_id', FILTER_SANITIZE_SPECIAL_CHARS));
        $msg = "";
        if(!empty($delete_invoice_id)) {	
            ...
            $bill_number = "";
            $bill_number = $obj->getTableColumnValue($GLOBALS['invoice_table'], 'invoice_id', $delete_invoice_id, 'bill_number');
        
            $action = "";
            if(!empty($bill_number)) {
                $action = "invoice Cancelled. Bill No. - ".$bill_number;
            }
            
            if(empty($delete_access_error)) {
                $delete_id =  $obj->DeletePayment($delete_invoice_id);
                $columns = array(); $values = array();
