<?php
	...
	if(isset($_REQUEST['show_voucher_id'])) {
        $show_voucher_id = filter_input(INPUT_GET, 'show_voucher_id', FILTER_SANITIZE_SPECIAL_CHARS);
        $voucher_date = date("Y-m-d"); 
        $selected_payment_mode = ""; $voucher_type = "";
        ...
        $party_list = array();
        $party_list = $obj->getPartyTypeList('Purchase');
                        ...
                        <div class="col-lg-3 col-md-3 col-6 party_display">
                            <div class="form-group pb-2">
                                <div class="form-label-group in-border mb-0">
                                    <select name="party_id" class="select2 select2-danger smallfnt" data-dropdown-css-class="select2 select2-danger" onchange="Javascript:HideDetails('Party', 'voucher');">
    ...
    if(isset($_POST['edit_id'])) {
        ...
        $form_name = "voucher_form"; $valid_voucher = ""; $voucher_type = ""; $bill_id = $bill_id_error = "";
        ...
        $access_error = ""; $permission_module = $GLOBALS['voucher_module']; $permission_action = "";
        ...
        if(empty($valid_voucher) && empty($payment_error)) {
			...
			if(preg_match("/^\d+$/", $check_user_id_ip_address)) {
                ...
                if(empty($edit_id)) {	
					...
					$columns = array('created_date_time','updated_date_time', 'creator', 'creator_name','bill_company_id', 'voucher_id', 'voucher_number', 'voucher_date', 'party_id', 'party_name','party_type','narration', 'amount', 'payment_mode_id', 'payment_mode_name', 'bank_id', 'bank_name','total_amount', 'deleted','name_mobile_city');
					$values = array($created_date_time,$updated_date_time, $creator, $creator_name,$bill_company_id, $null_value, $null_value, $voucher_date, $party_id, $party_name,$party_type,  $narration, $amount, $payment_mode_ids, $payment_mode_names, $bank_ids, $bank_names, $total_amount, '0',$name_mobile_city);
    ...
    if(isset($_POST['draw'])){
        ...
        $totalRecords = count($obj->getVoucherList($row, $rowperpage, $order_column, $order_direction, $search_text,$cancelled,$filter_from_date,$filter_to_date,$filter_party_id ));
       
        $filteredRecords = count($obj->getVoucherList('', '',$order_column, $order_direction, $search_text,$cancelled,$filter_from_date,$filter_to_date,$filter_party_id ));

        $data = [];

        $partyList = $obj->getVoucherList($row, $rowperpage, $order_column, $order_direction, $search_text,$cancelled,$filter_from_date,$filter_to_date,$filter_party_id );
        
        $sno = $row + 1;
        foreach ($partyList as $val) {
            $created_date_time = ""; $voucher_date = ""; $party_name = ""; $voucher_number =""; $total_amount ="";
            ...
            if(!empty($val['voucher_date']) && $val['voucher_date'] != $GLOBALS['null_value']) {
                $voucher_date = date('d-m-Y', strtotime($val['voucher_date']));
            }
            if(!empty($val['party_name']) && $val['party_name'] != $GLOBALS['null_value']){
                $party_name = $obj->encode_decode("decrypt",$val['party_name']);
            }
            if(!empty($val['voucher_number']) && $val['voucher_number'] != $GLOBALS['null_value']){
            ...
            $edit_access_error = ""; $permission_module = $GLOBALS['voucher_module']; $permission_action = $edit_action;
            if(!empty($login_staff_id)) {
                include('permission_action.php');
            }
            $delete_access_error = ""; $permission_module = $GLOBALS['voucher_module']; $permission_action = $delete_action;
            if(!empty($login_staff_id)) {
                include('permission_action.php');
            }
            if(empty($edit_access_error)){
                // $edit_option = '<li><a class="dropdown-item" href="Javascript:ShowModalContent('.'\''.$page_title.'\''.', '.'\''.$val['voucher_id'].'\''.');"><i class="fa fa-pencil"></i>&nbsp; Edit</a></li>';
            }
            ...
            if(empty($delete_access_error) && empty($val['deleted'])){
                if(!empty($linked_count)) { 
                    ...
                    $delete_option = '<li><a class="dropdown-item" href="Javascript:DeleteModalContent('.'\''.$page_title.'\''.', '.'\''.$val['voucher_id'].'\''.');"><i class="fa fa-trash"></i>&nbsp; Delete</a></li>';
                }
            }
            $print_option = '<li><a class="dropdown-item" target="_blank" href="reports/rpt_voucher.php?view_voucher_id=' . $val['voucher_id'] . '"><i class="fa fa-print"></i>&nbsp; Print</a></li>';
            
            if(empty($edit_access_error) || empty($delete_access_error)){
                $action = '<div class="dropdown">
                            <a href="#" role="button" class="btn btn-dark py-1 px-2" id="dropdownMenuLink'.$val['voucher_id'].'" data-bs-toggle="dropdown" aria-expanded="false">
                                <i class="bi bi-three-dots-vertical"></i>
                            </a>
                            <ul class="dropdown-menu" aria-labelledby="dropdownMenuLink'.$val['voucher_id'].'">
    ...
    if(isset($_REQUEST['delete_voucher_id'])) {
        $delete_voucher_id = filter_input(INPUT_GET, 'delete_voucher_id', FILTER_SANITIZE_SPECIAL_CHARS);
        $msg = "";
        if(!empty($delete_voucher_id)) {
            $access_error = ""; $permission_module = $GLOBALS['voucher_module']; $permission_action = $delete_action;
            if(!empty($login_staff_id)) {
                include('permission_action.php');
            }
            if(empty($access_error)) {
                $unique_id = "";
                $unique_id = $obj->getTableColumnValue($GLOBALS['voucher_table'], 'voucher_id', $delete_voucher_id, 'id');
                if(preg_match("/^\d+$/", $unique_id)) {
                    ...
                    $values = array(1);
                    $msg = $obj->UpdateSQL($GLOBALS['voucher_table'], $unique_id, $columns, $values, $action);
                }
            }
            else {
