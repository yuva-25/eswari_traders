<?php
    ...
    $view_invoice_id = ""; 
    if (isset($_REQUEST['view_invoice_id'])) {
        $view_invoice_id = $_REQUEST['view_invoice_id'];
    }else {
        header("Location: ../invoice.php");
        exit;
    }  

    $invoice_date = date('d-m-Y'); $bill_number = ""; $party_id = ""; $godown_type = "";  $product_count = 0; $gst_option = 2; $tax_option = 1;$tax_type = 2;$party_state = ""; $company_state = "";
    ...
    if(!empty($view_invoice_id)){
            ...
            if(!empty($bank_list)){
                foreach($bank_list as $bank){
                    if(!empty($bank['account_name']) && $bank['account_name'] != $GLOBALS['null_value']){
                        $account_name = $obj->encode_decode('decrypt',$bank['account_name']);
                    }
                    if(!empty($bank['bank_name_account_number']) && $bank['bank_name_account_number'] != $GLOBALS['null_value']){
                        $bank_name = $obj->encode_decode('decrypt',$bank['bank_name_account_number']);
                    }
                    // if(!empty($bank['account_number']) && $bank['account_number'] != $GLOBALS['null_value']){
                    //     $account_number = $obj->encode_decode('decrypt',$bank['account_number']);
                    // }
                    if(!empty($bank['ifsc_code']) && $bank['ifsc_code'] != $GLOBALS['null_value']){
                        $ifsc_code = $obj->encode_decode('decrypt',$bank['ifsc_code']);
                    }
                    if(!empty($bank['branch']) && $bank['branch'] != $GLOBALS['null_value']){
                        $branch = $obj->encode_decode('decrypt',$bank['branch']);
                    }
                }
            }
        }
        ...
        function combineAndSumUp ($myArray) {
            ...
            foreach ($myArray as $nkey => $nvalue) {
                ...
                foreach ($finalArray as $fkey => $fvalue) {
                    if (($fvalue['product_tax'] == $nvalue['product_tax'])) {
                        $has = true;
                        $fk = $fkey;
                        break;
                    }
                ...
                if ( $has === false ) {
                    ...
                    $finalArray[$fk]['product_amount'] += $nvalue['product_amount'];
                    $finalArray[$fk]['product_quantity'] += $nvalue['product_quantity'];
                }
            }
            return $finalArray;
        }
        ...
        $pdf->SetY($bill_to_y);
        $pdf->SetX(113);
        $pdf->Cell(80,8,'Invoice No.                :  ',0,0,'L',0);

        $pdf->SetFont('Arial','',9);
        $pdf->SetX(153);
        $pdf->Cell(40,8,$invoice_number,0,1,'L',0);
        ...
        $pdf->SetFont('Arial','',9);
        $pdf->SetX(153);
        $pdf->Cell(40,8,date('d-m-Y',strtotime($invoice_date)),0,1,'L',0);
        $bill_to_y2 = $pdf->GetY();
        ...
        $tot_subtotal = 0; $tax_plus_value1 = 0; $tax_plus_value2 = 0; $tot_cgst = 0; $tot_sgst = 0; $tot_igst = 0; $grand_product_amount = 0;

        if(!empty($view_invoice_id) && !empty($product_ids)) { 
            for($p = 0; $p < count($product_ids); $p++) { 
                ...
                elseif($tax_type == '1'){
                 ...
                 if($tax_type == '1'){
                    $product_tax[$p] = str_replace("%", '', $product_tax[$p]);

                    if(!empty($total_tax_value)){
                        if($company_state == $party_state) { 

                            $tax_plus_value1 = $total_tax_value/2 ;
                        }
                        else{
                            $tax_plus_value2 = $total_tax_value;
                        }
                    }

                    $tot_subtotal += $product_amount[$p];
                    $tot_cgst += $tax_plus_value1;
                    $tot_sgst += $tax_plus_value1;
                    $tot_igst += $tax_plus_value2;


                    $individual_tax[$p] = array('product_hsn_sac' => $product_tax[$p], 'product_amount' => ($product_amount[$p]), 'product_tax' => $product_tax[$p], 'product_tax_amount' => $tax_plus_value1,'product_quantity' => $product_quantity[$p]);
                }
            }
        ...
        if($tax_option == '2' && $tax_type == '2'){
            ...
            $sub_total_y = $pdf->GetY();

            if(!empty($discount_value)){
                $pdf->SetX(130);
                $pdf->SetFont('Arial','B',8.5);
                $pdf->Cell(40,6,$discount_name. " (" .$discount. ")",1,0,'R',0);
                $pdf->SetFont('Arial','',8.5);
                $pdf->Cell(30,6,$discount_value,1,1,'R',0);
                $pdf->SetX(130);
                $pdf->SetFont('Arial','B',8.5);
                $pdf->Cell(40,6,'Discounted Total',1,0,'R',0);
                $pdf->SetFont('Arial','',8.5);
            ...
            if(!empty($extra_charges_value)){
                $pdf->SetX(130);
                $pdf->SetFont('Arial','B',8.5);
                $pdf->Cell(40,6,$extra_charges_name. " (" .$extra_charges. ")",1,0,'R',0);
                $pdf->SetFont('Arial','',8.5);
                $pdf->Cell(30,6,$extra_charges_value,1,1,'R',0);

                $pdf->SetX(130);
                $pdf->SetFont('Arial','B',8.5);
                $pdf->Cell(40,6,'Charges Total',1,0,'R',0);
                $pdf->SetFont('Arial','',8.5);
            ...
            if($company_state == $party_state){
                ...
                if(!empty($gst_option)){
                    $pdf->SetX(130);
                    $pdf->SetFont('Arial','B',8.5);
                    $pdf->Cell(40,6,'CGST'. $tax,1,0,'R',0);
                    $pdf->SetFont('Arial','',8.5);
                    $pdf->Cell(30,6,number_format($cgst_value,2),1,1,'R',0);   
                    $pdf->SetX(130);
                    $pdf->SetFont('Arial','B',8.5);
                    $pdf->Cell(40,6,'SGST'. $tax,1,0,'R',0);
                    $pdf->SetFont('Arial','',8.5);
            ...
            if(!empty($round_off)){
                $pdf->SetX(130);
                $pdf->SetFont('Arial','B',8.5);
                $pdf->Cell(40,6,'Round Off',1,0,'R',0);
                $pdf->SetFont('Arial','',8.5);
        ...
        if(!empty($bank_id)){
            ...
            $pdf->SetY($sub_total_y);
            $pdf->SetX(10);
            $pdf->Cell(65,5,'Bank Details :-',0,1,'L');
            if(!empty($bank_name)){
                $pdf->SetFont('Arial','',8);
                $pdf->SetX(10);
                $pdf->Cell(30,3,'Bank Name          : ',0,0,'L');
                $pdf->MultiCell(30,3,$bank_name,0,'L');
            }
            if(!empty($account_name)){
                $pdf->SetFont('Arial','',8);
                $pdf->SetX(10);
                $pdf->Cell(30,4,'Account Name     : ',0,0,'L');
                $pdf->MultiCell(30,4,$account_name,0,'L');
            }
            if(!empty($account_number)){
                $pdf->SetFont('Arial','',8);
                $pdf->SetX(10);
                $pdf->Cell(30,4,'Account Number  : ',0,0,'L');
                $pdf->MultiCell(35,4,$account_number,0,'L');
            }
            if(!empty($ifsc_code)){
                $pdf->SetFont('Arial','',8);
                $pdf->SetX(10);
                $pdf->Cell(30,4,'IFSC                    : ',0,0,'L');
                $pdf->MultiCell(30,4,$ifsc_code,0,'L');
            }
            if(!empty($branch)){
                $pdf->SetFont('Arial','',8);
                $pdf->SetX(10);
                $pdf->Cell(30,4,'Branch                 : ',0,0,'L');
                $pdf->MultiCell(30,4,$branch,0,'L');
            }
        }
        $bank_y = $pdf->GetY();
        $max_foot_array = max(array($bill_end_y, $bank_y));
        if($tax_option == 2){
            $pdf->SetY($sub_total_y);
            $pdf->Cell(120,$max_foot_array - $sub_total_y,'',1,0,'R',0);
            $pdf->Cell(0,$max_foot_array - $sub_total_y,'',1,1,'R',0);
        }else{
            $pdf->SetY($sub_total_y);
            $pdf->Cell(120,$max_foot_array - $sub_total_y,'',1,0,'R',0);
            $pdf->Cell(0,$max_foot_array - $sub_total_y,'',1,1,'R',0);
        ...
        $pdf->SetFont('Arial','B',8);
        $pdf->SetX(10);
        $pdf->Cell(150,5,getIndianCurrency($bill_total),0,1,'L',0);
        $y31=$pdf->GetY();
        $pdf->SetY($y3);
        ...
        if($tax_type == 1){
            ...
            $greater_tax = 0; $harges_tax=0;
            if(!empty($extra_charges)) {
                // if($gst_option == "1"){
                    $tax_count = 0; $greater_tax = 0; $taxes_amount = 0; $str_tax = 0;
                    if(!empty($product_tax))
                    {
                        $tax_count = count($product_tax);
                        $tax = $product_tax;
                        for($i=0;$i<$tax_count;$i++)
                        {
                            $taxes_amount = trim(str_replace("%", "", $tax[$i]));
                            // echo $tax_amount ;
                            if($taxes_amount > $str_tax)
                            {
                                $greater_tax = $taxes_amount;
                            }
                            else{
                                $greater_tax = $str_tax;
                            }
                    ...
                    if (strpos($extra_charges, '%') !== false) {
                        $extra_charges = trim(str_replace("%", "", $extra_charges));
                    }else{
                        $extra_charges = trim(str_replace("Rs.", "", $extra_charges));
                    }
                    $charges_tax = ($extra_charges_value * $greater_tax)/100;
                    // echo $charges_tax."-Charge Tax<br>";
                // }
            }

            // $grand_product_amount -= $discounted_total;
            ...
            $pdf->SetFont('Arial','B',8);
            $pdf->SetX(10);
            $pdf->Cell(20,10,'Tax',1,0,'C',0);
            $pdf->SetX(30);
            $pdf->Cell(35,10,'Taxable Value',1,0,'C',0);
            // if(!empty($cgst_value) && !empty($party_state)) {
                $y=$pdf->GetY();
                if($company_state == $party_state) {
                    $pdf->SetX(65);
                    $pdf->Cell(52,5,'Central Tax',1,1,'C',0);
                    $pdf->SetX(65);
                    $pdf->Cell(26,5,'Tax',1,0,'C',0);
                    $pdf->SetX(91);
                    $pdf->Cell(26,5,'Amount',1,0,'C',0);
                    $pdf->SetY($y);
                    $pdf->SetX(117);
                    $pdf->Cell(52,5,'State Tax',1,1,'C',0);
                    $pdf->SetX(117);
                    $pdf->Cell(26,5,'Tax',1,0,'C',0);
                    $pdf->SetX(143);
                    $pdf->Cell(26,5,'Amount',1,0,'C',0);
                    $pdf->SetY($y);
                    $pdf->SetX(169);
                    $pdf->Cell(31,5,'Total',1,1,'C',0);
                    $pdf->SetX(169);
                    $pdf->Cell(31,5,'Tax Amount',1,1,'C',0);
                }
                else{
                    $y=$pdf->GetY();
                    $pdf->SetX(65);
                    $pdf->Cell(72,5,'Integrated Tax',1,1,'C',0);
                    $pdf->SetX(65);
                    $pdf->Cell(36,5,'Tax',1,0,'C',0);
                    $pdf->SetX(101);
                    $pdf->Cell(36,5,'Amount',1,0,'C',0);
                    $pdf->SetY($y);
                    $pdf->SetX(137);
                    $pdf->Cell(63,5,'Total',1,1,'C',0);
                    $pdf->SetX(137);
                    $pdf->Cell(63,5,'Tax Amount',1,1,'C',0);
                }
            // }

            $grand_tax_amount = 0; $grand_quantity = 0;
            $pdf->SetFont('Arial','',8);
            $yaxis=$pdf->GetY();
            foreach($final_array as $tax_data) {
                $strtax_amount = 0; $discount_cal = "";
                $pdf->SetX(10);
                $pdf->Cell(20,5,$tax_data['product_tax'].'%',0,0,'C',0);
                $tax = trim(str_replace("%", "", $tax_data['product_tax']));
                $pdf->SetX(30);
                if(!empty($discount)){
                    $product_discount = 0;
                    if (strpos($discount, '%') !== false) {
                        $discount_cal = trim(str_replace("%", "", $discount));
                        $product_discount = $discount_cal;
                    }else{
                        $discount = trim(str_replace("Rs.", "", $discount));
                        $product_discount = round((($discount / $sub_total) * 100),2);
                    }

                    
                    // $tax_data['product_amount'] = round($tax_data['product_amount'] - (($tax_data['product_amount'] * $product_discount) / 100),2);

                    $tax_data['product_amount'] = round($tax_data['product_amount'] - (($tax_data['product_amount'] * $product_discount) / 100),2);

                    // $tax_data['product_amount'] = $discounted_total;
                    $grand_product_amount += $tax_data['product_amount'];
                }
            
                
                $pdf->Cell(35,5,$obj->numberFormat($tax_data['product_amount'],2),0,0,'R',0);
                $tax = 0;
                $tax = trim(str_replace("%", "", $tax_data['product_tax']));
                $percentage = 100;
                // if($tax_option == "1"){
                //     $percentage = 100 + $tax;
                // }
                // print_r($tax_data['product_amount']);
                $strtax_amount = (($tax_data['product_amount'])*$tax)/$percentage;


                if(!empty($company_state) && !empty($party_state)) {
                    
                    if($company_state == $party_state) {
                        // echo "statesss :".$company_state." / ".$party_state;
                        $pdf->SetX(65);
                        $pdf->Cell(26,5,($tax/2)."%",0,0,'C',0);
                        $pdf->SetX(91);
                        $pdf->Cell(26,5,$obj->numberFormat($strtax_amount/2,2),0,0,'R',0);
                        $pdf->SetX(117);
                        $pdf->Cell(26,5,($tax/2)."%",0,0,'C',0);
                        $pdf->SetX(143);
                        $pdf->Cell(26,5,$obj->numberFormat($strtax_amount/2,2),0,0,'R',0);
                        $pdf->SetX(169);
                        $pdf->Cell(31,5,$obj->numberFormat($strtax_amount,2),0,1,'R',0);
                        $strtax_amount = $strtax_amount;
                    }
                    else
                    {
                        $pdf->SetX(65);
                        $pdf->Cell(36,5,$tax.'%',0,0,'C',0);
                        $pdf->SetX(101);
                        $pdf->Cell(36,5,$obj->numberFormat($strtax_amount,2),0,0,'C',0);
                        $pdf->SetX(137);
                        $pdf->Cell(63,5,$obj->numberFormat($strtax_amount,2),0,1,'R',0);
                    }
                }
                $no_of_products = 0;
                $strproduct_amount = 0;
                ...
                $no_of_products++;
                $grand_tax_amount += $strtax_amount;
                $grand_product_amount += $tax_data['product_amount'];
                $grand_quantity += $tax_data['product_quantity'];
            }


            if(!empty($extra_charges_value) && $extra_charges_value != ''){
                $pdf->SetX(10);
                $pdf->Cell(20,5,'Chrages',0,0,'C',0);

                $pdf->SetX(30);
                $pdf->Cell(35,5,$extra_charges_value,0,0,'R',0);
                $grand_product_amount += $extra_charges_value;

                if($company_state == $party_state){
                    $pdf->SetX(65);
                    $pdf->Cell(26,5,($greater_tax/2).'%',0,0,'C',0);

                    $pdf->SetX(91);
                    $pdf->Cell(26,5,$obj->numberFormat(($charges_tax/2),2),0,0,'R',0);
                    ...
                    $grand_tax_amount += $charges_tax;

                    $pdf->SetX(137);
                    $pdf->Cell(63,5,$obj->numberFormat($charges_tax, 2),0,1,'R',0);
                } else {

                    $pdf->SetX(65);
                    $pdf->Cell(36,5,($greater_tax).'%',0,0,'C',0);

                    $pdf->SetX(101);
                    $pdf->Cell(36,5,$obj->numberFormat(($charges_tax),2),0,0,'C',0);
            ...
            $pdf->SetX(30);
            $pdf->Cell(35,30,'',1,0,'R',0);
            // if(!empty($company_state) && !empty($party_state)) {
                if($company_state == $party_state) {
                    $pdf->SetX(65);
                    $pdf->Cell(26,30,'',1,0,'C',0);
                    $pdf->SetX(91);
                    $pdf->Cell(26,30,'',1,0,'R',0);
                    $pdf->SetX(117);
                    $pdf->Cell(26,30,'',1,0,'C',0);
                    $pdf->SetX(143);
                    $pdf->Cell(26,30,'',1,0,'R',0);
                    $pdf->SetX(169);
                    $pdf->Cell(31,30,'',1,1,'R',0);
                }
                else
                {
                    $pdf->SetX(65);
                    $pdf->Cell(36,30,'',1,0,'C',0);
                    $pdf->SetX(101);
                    $pdf->Cell(36,30,'',1,0,'C',0);
                    $pdf->SetX(137);
                    $pdf->Cell(63,30,'',1,1,'R',0);
                }
            // }
            ...
            $pdf->SetX(30);
            $pdf->Cell(35,5,$obj->numberFormat($taxable_value,2),1,0,'R',0);
            ...
            // if(!empty($company_state) && !empty($party_state)) {
                if($company_state == $party_state) {
                    ...
                    $pdf->SetX(91);
                    $pdf->Cell(26,5,$obj->numberFormat($grand_tax_amount/2,2),1,0,'R',0);
                    $pdf->SetX(117);
                    $pdf->Cell(26,5,'',1,0,'C',0);
                    $pdf->SetX(143);
                    $pdf->Cell(26,5,$obj->numberFormat($grand_tax_amount/2,2),1,0,'R',0);
                    $pdf->SetX(169);
                    $pdf->Cell(31,5,$obj->numberFormat($grand_tax_amount,2),1,1,'R',0);
                }
                else
                {
                    $pdf->SetX(65);
                    $pdf->Cell(36,5,'',1,0,'C',0);
                    $pdf->SetX(101);
                    $pdf->Cell(36,5,$obj->numberFormat($grand_tax_amount,2),1,0,'C',0);
                    $pdf->SetX(137);
                    $pdf->Cell(63,5,$obj->numberFormat($grand_tax_amount,2),1,1,'R',0);
                }
            // }
            $y_start_y=$pdf->GetY();
            $pdf->SetFont('Arial','',8);
            $pdf->SetX(10);
            $pdf->Cell(30,5,'Tax Amount (in words) : ',0,0,'L',0);
            $pdf->SetX(40);
            $pdf->SetFont('Arial','B',8);
            $pdf->Cell(150,5,getIndianCurrency($grand_tax_amount),0,1,'L',0);
            $y_end_y = $pdf->GetY();

            $pdf->SetY($y_start_y);
            $pdf->SetX(10);
            $pdf->Cell(0,$y_end_y - $y_start_y,'',1,1,'C',0);
        }
