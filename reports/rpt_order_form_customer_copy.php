<?php
  ...
  $product_ids = array(); $unit_ids = array(); $size_ids = array(); $quantity = array(); $rates = array(); $amounts = array(); $party_id = ""; $admin = 0; $notes = "";   $descriptions = array(); $hsn_codes = array();
    ...
    if(!empty($view_order_form_id)){
        ...
        $pdf->Cell(60,8,'Product Name',1,0,'C',1);
        $pdf->Cell(30,8,'QTY/Unit',1,0,'C',1);  
        ...
        if(!empty($view_order_form_id) && !empty($product_ids)) { 
            for($p = 0; $p < count($product_ids); $p++) { 
                ...
                if(!empty($product_names)){
                    $pdf->MultiCell(60, 7, html_entity_decode($product_names[$p]), 0, 'L');
                }else{
                    $pdf->MultiCell(60, 7, " - ", 0, 'L');
                }
                ...
                if(!empty($hsn_codes[$p]) || !empty($description[$p])){
                    ...
                    if(!empty($hsn_codes[$p])){
                        $pdf->SetX(20);
                        $pdf->MultiCell(60, 3,"HSN Code : ".$hsn_codes[$p], 0, 'L');
                    }
                    if(!empty($description[$p])){
                        $pdf->SetX(20);
                        $pdf->MultiCell(60, 3,"Description : ".$description[$p], 0, 'L');
                    }
                    $pdf->SetTextColor(0, 0, 0);
                    $pdf->SetFont('Arial', '', 8);
                }
                ...
                $pdf->SetY($start_y);
                $pdf->SetX(80);
                if(!empty($unit_names[$p]) && !empty($quantity[$p])){
                    $pdf->MultiCell(30, 7,  $quantity[$p]." ".$unit_names[$p], 0, 'C');
                }else{
                ...
                $pdf->SetX(140);
                if(!empty($rates[$p])){
                    $pdf->MultiCell(30, 7,  $rates[$p], 0, 'R');
                }else{
                    $pdf->MultiCell(30, 7,  '-', 0, 'C');
                }
                $rate_y = $pdf->GetY() - $start_y;
                
                $pdf->SetY($start_y);
                $pdf->SetX(170);
                if(!empty($amounts[$p])){
                    $pdf->MultiCell(30, 7,number_format($amounts[$p], 2), 0, 'R');
                    $total_amount += $amounts[$p];
                } else {
                    $pdf->MultiCell(30, 7,'-', 0, 'C');
                }
                $amount_y = $pdf->GetY() - $start_y;


                $y_array = array($product_names_y,$unit_names_y, $size_name_y,$rate_y,$amount_y);
        ...
        if (($footer_height + $end_y) > 270) { 
            ...
            $pdf->Cell(60,(280 - $y_axis),'',1,0,'C',0);
        ...
        if(!empty($total_amount)){
            ...
            $pdf->Cell(30,6.5,number_format($total_amount, 2),1,1,'R',0);
        }
