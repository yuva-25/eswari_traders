<?php
    ...
    $pdf_download_name ="";
    $pdf_download_name = "Order Form Report PDF -"." (".$from_date ." to ".$to_date .")";
    ...
    $file_name="Order Form Report";
    include("rpt_header.php");
    
    $pdf->SetY($header_end);
           
    $pdf->SetFont('Arial','B',10);
    $pdf->Cell(0,7.5,'Order Form Report ('.$from_date.' - '.$to_date.')',0,1,'C',0);
    ...
    $pdf->Cell(60,8,'Party Name',1,0,'C',1);
    ...
    if(!empty($total_records_list)) {    
        foreach($total_records_list as $key => $data) {
            ...
            if(!empty($data['order_form_number'])) {
                if ($data['cancelled'] == '1') {
                    $pdf->SetX(20);
                    $pdf->MultiCell(35,7,$data['order_form_number'],0,'C',0);
                }else{ 
                    $pdf->SetX(20);
                    $pdf->MultiCell(35,7,$data['order_form_number'],0,'C',0); 
                }
            ...
            if ($data['cancelled'] == '1') {
                ...
                $pdf->SetX(20);
                $pdf->Cell(35,7,'Cancelled',0,1,'C',0);   
            }
            $pdf->SetTextColor(0,0,0);
            $no_end = $pdf->GetY();
            ...
            if(!empty($data['order_form_date'])) {
                $pdf->SetX(55);
                $pdf->MultiCell(35,7,date('d-m-Y',strtotime($data['order_form_date'])),0,'C',0);
            }
            $date_end = $pdf->GetY();
            ...
            if(!empty($data['party_id'])) {
                ...
                $pdf->SetX(90);
                $pdf->MultiCell(60,7,$obj->encode_decode('decrypt',$customer_name),0,'C',0);
            }
