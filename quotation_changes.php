<?php
	...
	if(isset($_REQUEST['show_quotation_id'])) { 
        $show_quotation_id = filter_input(INPUT_GET, 'show_quotation_id', FILTER_SANITIZE_SPECIAL_CHARS);

        $show_quotation_id = trim($show_quotation_id);
        ...
        $quotation_date = date('Y-m-d'); $bill_number = ""; $party_id = ""; $product_count = 0; $gst_option = 2; $tax_option = 1;$tax_type = "";$party_state = ""; $company_state = "";
        ...
        <form class="poppins pd-20 redirection_form" name="quotation_form" method="POST">
                ...
                <div class="col-lg-2 col-md-4 col-6 py-2 px-lg-1">
                    <div class="form-group">
                        <div class="form-label-group in-border">
                            <input type="date" id="quotation_date" name="quotation_date" class="form-control shadow-none" placeholder="" value="<?php if(!empty($quotation_date)){ echo $quotation_date; }?>" max="<?php if(!empty($current_date)){ echo $current_date; }?>">
            ...
            <div class="row justify-content-center">    
                ...
                <div class="col-md-12 py-3 text-center">
                    <button class="btn btn-danger submit_button" type="button" onclick="Javascript:SaveModalContent(event, 'quotation_form', 'quotation_changes.php', 'quotation.php');">
                        Submit
                    </button>
                </div>
            </div>
      ...
      if(isset($_POST['edit_id'])) {
        $quotation_date = ""; $quotation_date_error = "";$bill_number = ""; $bill_number_error = ""; $party_id = ""; $party_id_error = ""; $product_ids = array(); $product_names = array(); $row_no = array(); $unit_ids = array(); $unit_names = array();$hsn_code = array(); $quantity = array();$rates = array(); $product_tax = array(); $size_ids = array(); $amount = array(); $sub_total = 0;  $notes = ""; $total_quantity = 0;$individual_array = array();
        $form_name = "quotation_form"; $quotation_error = ""; $total_amount = 0;
        ...
        if(isset($_POST['quotation_date'])) {
            ...
            $quotation_date_error = $valid->valid_date(date('Y-m-d', strtotime($quotation_date)), 'Bill Date', '1');
            ...
            if(!empty($notes)){
              $notes_error = $valid->common_validation($notes, 'Notes', 'select');
                if(!empty($notes_error)) {
                    if(!empty($valid_quotation)) {
                        $valid_quotation = $valid_quotation." ".$valid->error_display($form_name, 'notes', $notes_error, 'select');
                    }
                    else {
                        $valid_quotation = $valid->error_display($form_name, 'notes', $notes_error, 'select');
                    }
                }
                else {
        ...
        if(!empty($gst_option_error)) {
            if(!empty($valid_quotation)) {
                $valid_quotation = $valid_quotation." ".$valid->error_display($form_name, 'gst_option', $gst_option_error, 'input');
            }
            else {
                $valid_quotation = $valid->error_display($form_name, 'gst_option', $gst_option_error, 'input');
            }
        }
        ...
        if($gst_option == '1') {
            ...
            if(!empty($tax_type_error)) {
                if(!empty($valid_quotation)) {
                    $valid_quotation = $valid_quotation." ".$valid->error_display($form_name, 'tax_type', $tax_type_error, 'select');
                }
                else {
                    $valid_quotation = $valid->error_display($form_name, 'tax_type', $tax_type_error, 'select');
                }
            }
            ...
            if(!empty($tax_option_error)) {
                if(!empty($valid_quotation)) {
                    $valid_quotation = $valid_quotation." ".$valid->error_display($form_name, 'tax_option', $tax_option_error, 'select');
                }
                else {
                    $valid_quotation = $valid->error_display($form_name, 'tax_option', $tax_option_error, 'select');
                }
            }
            ...
            if($tax_type == '2') {
                ...
                if(!empty($overall_tax_error)) {
                    if(!empty($valid_quotation)) {
                        $valid_quotation = $valid_quotation." ".$valid->error_display($form_name, 'overall_tax', $overall_tax_error, 'select');
                    }
                    else {
                        $valid_quotation = $valid->error_display($form_name, 'overall_tax', $overall_tax_error, 'select');
                    }
                }
            }
        }
            ...
            for($i=0; $i < count($product_ids); $i++) {
                    ...
                    if(empty($quantity_error)){
                        ...
                        if(isset($rates[$i])) {
                            ...
                            if(empty($price_error)){
                                ...
                                if(!empty($valid_quotation)) {
                                    $valid_quotation = $valid_quotation." ".$valid->row_error_display($form_name, 'rates[]', $price_error, 'text', 'product_row', ($i+1));
                                }
                                else {
                                    $valid_quotation = $valid->row_error_display($form_name, 'rates[]', $price_error, 'text', 'product_row', ($i+1));
                                }
                            }
                        ...
                        if(!empty($valid_quotation)) {
                            $valid_quotation = $valid_quotation." ".$valid->row_error_display($form_name, 'quantity[]', $quantity_error, 'text', 'product_row', ($i+1));
                        }
                        else {
                            $valid_quotation = $valid->row_error_display($form_name, 'quantity[]', $quantity_error, 'text', 'product_row', ($i+1));
                        }
                    }
                ...
                if(isset($hsn_code[$i])) {
                    ...
                    if(!empty($hsn_error)){
                        ...
                        if(!empty($valid_quotation)) {
                            $valid_quotation = $valid_quotation." ".$valid->row_error_display($form_name, 'hsn_code[]', $hsn_error, 'text', 'product_row', ($i+1));
                        }
                        else {
                            $valid_quotation = $valid->row_error_display($form_name, 'hsn_code[]', $hsn_error, 'text', 'product_row', ($i+1));
                        }
                    }
                ...
                if(isset($description[$i])) {
                    ...
                    if(empty($description_error)){
                        ...
                        if(!empty($description[$i]) && !empty($description_error)){
                            ...
                            if(!empty($valid_quotation)) {
                                $valid_quotation = $valid_quotation." ".$valid->row_error_display($form_name, 'description[]', $description_error, 'text', 'product_row', ($i+1));
                            }
                            else {
                                $valid_quotation = $valid->row_error_display($form_name, 'description[]', $description_error, 'text', 'product_row', ($i+1));
                            }
                        }
                    }
        ...
        if(!empty($discount_name_error)) {
            if(!empty($valid_quotation)) {
                $valid_quotation = $valid_quotation." ".$valid->error_display($form_name, 'discount_name', $discount_name_error, 'text');
            }
            else {
                $valid_quotation = $valid->error_display($form_name, 'discount_name', $discount_name_error, 'text');
            }
        }
        if(!empty($discount)) {
        ...
        if(!empty($discount_error)) {
            if(!empty($valid_quotation)) {
                $valid_quotation = $valid_quotation." ".$valid->error_display($form_name, 'discount', $discount_error, 'text');
            }
            else {
                $valid_quotation = $valid->error_display($form_name, 'discount', $discount_error, 'text');
            }
        }
        ...
        if(!empty($extra_charges_name_error)) {
            if(!empty($valid_quotation)) {
                $valid_quotation = $valid_quotation." ".$valid->error_display($form_name, 'extra_charges_name', $extra_charges_name_error, 'text');
            }
            else {
                $valid_quotation = $valid->error_display($form_name, 'extra_charges_name', $extra_charges_name_error, 'text');
            }
        }
        if(!empty($extra_charges)) {
        ...
        if(!empty($extra_charges_error)) {
            if(!empty($valid_quotation)) {
                $valid_quotation = $valid_quotation." ".$valid->error_display($form_name, 'extra_charges', $extra_charges_error, 'text');
            }
            else {
                $valid_quotation = $valid->error_display($form_name, 'extra_charges', $extra_charges_error, 'text');
            }
        }
        $extra_charges_total = $total_amount;

        $greater_tax = ""; $str_tax = ""; $extra_charges_tax = 0; $product_tax_value = array(); $stock_unique_ids = array();
        if($gst_option == '1' && empty($quotation_error) && empty($valid_quotation)) {
            ...
            if($tax_type =='1'){
                ...
                if(!empty($overall_tax)) {
                    ...
                    if(preg_match("/^[0-9]+(\\.[0-9]+)?$/", $tax)) {
                        ...
                        $quotation_error = "Invalid Overall tax";
                    }
                    if(!empty($product_ids) && empty($quotation_error) && empty($valid_quotation)) {
                        for($j=0; $j < count($product_ids); $j++) {
                            $discounted_product_amount = 0; $tax_value = 0;
                            if(!empty($product_amount[$j])) {
                                $discounted_product_amount = $product_amount[$j];
        ...
        if(empty($quotation_error) && empty($valid_quotation) && $total_amount <= '0') {
            $quotation_error = "Bill value cannot be 0";
        }
            if(!empty($edit_id) && empty($quotation_error) && empty($valid_quotation)) {
        ...
        if(empty($quotation_error) && empty($valid_quotation)){
            ...
            if(preg_match("/^\d+$/", $check_user_id_ip_address)) {
                ...
                if(empty($edit_id)){
                    ...
                    if(preg_match("/^\d+$/", $quotation_insert_id)) {
                        ...
                        $quotation_number = "";
                        $quotation_number = $obj->getTableColumnValue($GLOBALS['quotation_table'],'quotation_id',$quotation_id,'quotation_number');

                        $result = array('number' => '1', 'msg' => 'Quotation Successfully Created','redirection_page' =>'quotation.php','quotation_id' => $quotation_id);
                    }
                    else {
                ...
                // if($update_stock == '1' && !empty($quotation_id) && !empty($quotation_number)) {
                //     $remarks = $quotation_number;
                ...
                //     if(!empty($product_ids)) {
                //         for($i=0; $i < count($product_ids); $i++) {
                //             $stock_update = $obj->StockUpdate($GLOBALS['quotation_table'], 'Out', $quotation_id, $quotation_number, $product_ids[$i], $unit_ids[$i],$size_ids[$i], $remarks, $quotation_date, $quantity[$i],$party_id); 
                //         }
                //     }
                // }
                /* if($balance == 1 ) {
                    if(!empty($party_id)) {
                        $credit  = 0; $debit = 0; $bill_type ="Quotation";
                        $bill_id = $party_id;
                        $bill_date = $quotation_date;
                        $bill_number =  $quotation_number;
                        $credit = 0;
                        $debit = $bill_total;
    ...
    if(isset($_POST['draw'])){
        ...
        $totalRecords = 0;
        $totalRecords = count($obj->getQuotationRecordList($row, $rowperpage, $searchValue, $filter_from_date, $filter_to_date, $filter_party_id, $cancelled, $order_column, $order_direction));
        $filteredRecords = count($obj->getQuotationRecordList('', '', $searchValue, $filter_from_date, $filter_to_date, $filter_party_id, $cancelled, $order_column, $order_direction));

        $data = [];
        
        $quotationList = $obj->getQuotationRecordList($row, $rowperpage, $searchValue, $filter_from_date, $filter_to_date, $filter_party_id, $cancelled, $order_column, $order_direction);
        $sno = $row + 1;
        foreach ($quotationList as $val) {
            $created_date_time = $by_creator_name = ""; $updated_date_time = ""; $quotation_date = ""; $bill_number = ""; $party_name = "";$party_mobile_number =  $bill_total = 0;$quotation_number =""; $party_name_mobile_city ='';$product_count =0 ;
            ...
            if(!empty($val['quotation_date']) && $val['quotation_date'] != "0000-00-00") {
                $quotation_date = date('d-m-Y', strtotime($val['quotation_date']));
            }
            ...
            if(!empty($val['quotation_number']) && $val['quotation_number'] != $GLOBALS['null_value']) {
            ...
            $edit_access_error = ""; $permission_module = $GLOBALS['quotation_module']; $permission_action = $edit_action;
            if(!empty($login_role_id)) {
                include("user_permission_action.php");
            }
            $delete_access_error = ""; $permission_module = $GLOBALS['quotation_module']; $permission_action = $delete_action;
            if(!empty($login_role_id)) {
                include("user_permission_action.php");
            }
            if(empty($edit_access_error) && empty($val['cancelled'])) {
                $edit_option = '<li><a class="dropdown-item" href="Javascript:ShowModalContent('.'\''.$page_title.'\''.', '.'\''.$val['quotation_id'].'\''.');"><i class="fa fa-pencil"></i>&nbsp; Edit</a></li>';
            }
            if(empty($delete_access_error) && empty($val['cancelled'])) {
                $delete_option = '<li><a class="dropdown-item" href="Javascript:DeleteModalContent('.'\''.$page_title.'\''.', '.'\''.$val['quotation_id'].'\''.');"><i class="fa fa-ban"></i>&nbsp; Cancel</a></li>';
            }
            $conversion_option = "";
            $print_option = '<li><a class="dropdown-item" target="_blank" href="reports/rpt_quotation_a4.php?view_quotation_id=' . $val['quotation_id'] . '"><i class="fa fa-print"></i>&nbsp; A4 Print</a></li>';

            // if(empty($edit_access_error) && empty($val['cancelled'])) {
            //     $conversion_option = '<li><a class="dropdown-item" href="Javascript:ShowBillConversion('.'\''.$page_title.'\''.', '.'\''.$val['quotation_id'].'\''.');"><i class="bi bi-arrow-left-right"></i>&nbsp; Conversion</a></li>';
            // }
            $action = '<div class="dropdown">
                            <a href="#" role="button" class="btn btn-dark py-1 px-2" id="dropdownMenuLink'.$val['quotation_id'].'" data-bs-toggle="dropdown" aria-expanded="false">
                                <i class="bi bi-three-dots-vertical"></i>
                            </a>
                            <ul class="dropdown-menu" aria-labelledby="dropdownMenuLink'.$val['quotation_id'].'">
    ...
    if(isset($_REQUEST['delete_quotation_id'])) {
        $delete_quotation_id = trim(filter_input(INPUT_GET, 'delete_quotation_id', FILTER_SANITIZE_SPECIAL_CHARS));
        $msg = "";
        if(!empty($delete_quotation_id)) {	
            ...
            $order_form_unique_id = $obj->getTableColumnValue($GLOBALS['order_form_table'], 'conversion_id', $delete_quotation_id, 'id');
            if(preg_match("/^\d+$/", $quotation_unique_id)) {
                $bill_number = "";
                $bill_number = $obj->getTableColumnValue($GLOBALS['quotation_table'], 'quotation_id', $delete_quotation_id, 'bill_number');
            
                $action = "";
                if(!empty($bill_number)) {
                    $action = "quotation Cancelled. Bill No. - ".$bill_number;
                }
               
                if(empty($delete_access_error)) {
                    $stock_delete = '';
                    $stock_delete = $obj->DeletePrevList($delete_quotation_id, '');
                    if(empty($stock_delete == '1')) {
                        if(!empty($order_form_unique_id)){
                            $columns = array(); $values = array();
                            $columns = array('converted','conversion_id','conversion_number');
                            $values = array(0,$GLOBALS['null_value'],$GLOBALS['null_value']);
