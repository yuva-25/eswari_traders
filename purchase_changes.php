<?php
    ...
    if(isset($_SESSION[$GLOBALS['site_name_user_prefix'].'_user_id']) && !empty($_SESSION[$GLOBALS['site_name_user_prefix'].'_user_id'])) {
        if(!empty($GLOBALS['user_type']) && $GLOBALS['user_type'] != $GLOBALS['admin_user_type']) {
            ...
            $permission_module = $GLOBALS['purchase_module'];
        }
    }

	if(isset($_REQUEST['show_purchase_id'])) { 
        $show_purchase_id = filter_input(INPUT_GET, 'show_purchase_id', FILTER_SANITIZE_SPECIAL_CHARS);

        $show_purchase_id = trim($show_purchase_id);
        ...
        $purchase_date = date('Y-m-d'); $bill_number = ""; $party_id = ""; $product_count = 0; $gst_option = 2; $tax_option = 1;$tax_type = "";$party_state = ""; $company_state = "";
        ...
        if(!empty($show_purchase_id)) {
            $purchase_list = array();
            $purchase_list = $obj->getTableRecords($GLOBALS['purchase_table'], 'purchase_id', $show_purchase_id,'');
            $purchase_list = $obj->getTableRecords($GLOBALS['purchase_table'], 'purchase_id', $show_purchase_id,'');
		
            if(!empty($purchase_list)) {
                foreach($purchase_list as $data) {
        ...
        $party_list = array();
        $party_list = $obj->getPartyDetailList('1');
        $product_list = array();
        $product_list = $obj->getTableRecords($GLOBALS['product_table'], '', '', '');
        ?>
        <form class="poppins pd-20 redirection_form" name="purchase_form" method="POST">
			<div class="card-header">
				<div class="row p-2">
                    <div class="col-lg-8 col-md-8 col-8 align-self-center">
                        <?php if(empty($show_purchase_id)) { ?>
						    <div class="h5">Add Purchase</div>
                        <?php }else { ?>
						    <div class="h5">Edit Purchase</div>
                        <?php } ?>
					</div>
					<div class="col-lg-4 col-md-4 col-4">
                        <button class="btn btn-dark float-end" style="font-size:11px;" type="button" onclick="window.open('purchase.php','_self')"> <i class="fa fa-arrow-circle-o-left"></i> &ensp; Back </button>
            ...
            <div class="row justify-content-center p-3">
                ...
                <input type="hidden" name="edit_id" value="<?php if(!empty($show_purchase_id)) { echo $show_purchase_id; } ?>">
                <div class="col-lg-2 col-md-4 col-6 py-2 px-lg-1">
                    <div class="form-group">
                        <div class="form-label-group in-border">
                            <input type="date" id="purchase_date" name="purchase_date" class="form-control shadow-none" placeholder="" value="<?php if(!empty($purchase_date)){ echo $purchase_date; }?>" max="<?php if(!empty($current_date)){ echo $current_date; }?>">
                    ...
                    <div class="form-group">
                        <div class="form-label-group in-border">
                            <div class="input-group  flex-nowrap" >
                                <select class="select2 select2-danger" name ="party_id" data-dropdown-css-class="select2-danger" style="width: 100%;" onchange="Javascript:getPartyState(this.value);">
            ...
            <div class="row justify-content-center">    
                ...
                <div class="col-md-12 py-3 text-center">
                    <button class="btn btn-danger submit_button" type="button" onclick="Javascript:SaveModalContent(event, 'purchase_form', 'purchase_changes.php', 'purchase.php');">
                        Submit
                    </button>
                </div>
            </div>
      ...
      if(isset($_POST['edit_id'])) {
        $purchase_date = ""; $purchase_date_error = "";$bill_number = ""; $bill_number_error = ""; $party_id = ""; $party_id_error = ""; $product_ids = array(); $product_names = array(); $row_no = array(); $unit_ids = array(); $unit_names = array();$hsn_code = array(); $quantity = array();$rates = array(); $product_tax = array(); $size_ids = array(); $amount = array(); $sub_total = 0;  $notes = ""; $total_quantity = 0;$individual_array = array();
        $form_name = "purchase_form"; $purchase_error = ""; $total_amount = 0;
        ...
        if(isset($_POST['purchase_date'])) {
            ...
            $purchase_date_error = $valid->valid_date(date('Y-m-d', strtotime($purchase_date)), 'Bill Date', '1');
            if(!empty($purchase_date_error)) {
                if(!empty($valid_purchase)) {
                    $valid_purchase = $valid_purchase." ".$valid->error_display($form_name, 'purchase_date', $purchase_date_error, 'text');
                }
                else {
                    $valid_purchase = $valid->error_display($form_name, 'purchase_date', $purchase_date_error, 'text');
                }
            }
        }
            ...
            if(!empty($notes)){
              $notes_error = $valid->common_validation($notes, 'Notes', 'select');
                if(!empty($notes_error)) {
                    if(!empty($valid_purchase)) {
                        $valid_purchase = $valid_purchase." ".$valid->error_display($form_name, 'notes', $notes_error, 'select');
                    }
                    else {
                        $valid_purchase = $valid->error_display($form_name, 'notes', $notes_error, 'select');
                    }
                }
                else {
        ...
        if(!empty($gst_option_error)) {
            if(!empty($valid_purchase)) {
                $valid_purchase = $valid_purchase." ".$valid->error_display($form_name, 'gst_option', $gst_option_error, 'input');
            }
            else {
                $valid_purchase = $valid->error_display($form_name, 'gst_option', $gst_option_error, 'input');
            }
        }
        ...
        if($gst_option == '1') {
            ...
            if($tax_type == '2') {
                ...
                if(!empty($overall_tax_error)) {
                    if(!empty($valid_purchase)) {
                        $valid_purchase = $valid_purchase." ".$valid->error_display($form_name, 'overall_tax', $overall_tax_error, 'select');
                    }
                    else {
                        $valid_purchase = $valid->error_display($form_name, 'overall_tax', $overall_tax_error, 'select');
                    }
                }
            }
        }
            ...
            for($i=0; $i < count($product_ids); $i++) {
                    ...
                    if(empty($quantity_error)){
                        ...
                        if(isset($rates[$i])) {
                            ...
                            if(empty($price_error)){
                                ...
                                if(!empty($valid_purchase)) {
                                    $valid_purchase = $valid_purchase." ".$valid->row_error_display($form_name, 'rates[]', $price_error, 'text', 'product_row', ($i+1));
                                }
                                else {
                                    $valid_purchase = $valid->row_error_display($form_name, 'rates[]', $price_error, 'text', 'product_row', ($i+1));
                                }
                            }
                        ...
                        if(!empty($valid_purchase)) {
                            $valid_purchase = $valid_purchase." ".$valid->row_error_display($form_name, 'quantity[]', $quantity_error, 'text', 'product_row', ($i+1));
                        }
                        else {
                            $valid_purchase = $valid->row_error_display($form_name, 'quantity[]', $quantity_error, 'text', 'product_row', ($i+1));
                        }
                    }
                ...
                if(isset($hsn_code[$i])) {
                    ...
                    if(!empty($hsn_error)){
                        ...
                        if(!empty($valid_purchase)) {
                            $valid_purchase = $valid_purchase." ".$valid->row_error_display($form_name, 'hsn_code[]', $hsn_error, 'text', 'product_row', ($i+1));
                        }
                        else {
                            $valid_purchase = $valid->row_error_display($form_name, 'hsn_code[]', $hsn_error, 'text', 'product_row', ($i+1));
                        }
                    }
                ...
                if(isset($description[$i])) {
                    ...
                    if(empty($description_error)){
                        ...
                        if(!empty($description[$i]) && !empty($description_error)){
                            ...
                            if(!empty($valid_purchase)) {
                                $valid_purchase = $valid_purchase." ".$valid->row_error_display($form_name, 'description[]', $description_error, 'text', 'product_row', ($i+1));
                            }
                            else {
                                $valid_purchase = $valid->row_error_display($form_name, 'description[]', $description_error, 'text', 'product_row', ($i+1));
                            }
                        }
                    }
        ...
        if(!empty($discount_name_error)) {
            if(!empty($valid_purchase)) {
                $valid_purchase = $valid_purchase." ".$valid->error_display($form_name, 'discount_name', $discount_name_error, 'text');
            }
            else {
                $valid_purchase = $valid->error_display($form_name, 'discount_name', $discount_name_error, 'text');
            }
        }
        if(!empty($discount)) {
        ...
        if(!empty($discount_error)) {
            if(!empty($valid_purchase)) {
                $valid_purchase = $valid_purchase." ".$valid->error_display($form_name, 'discount', $discount_error, 'text');
            }
            else {
                $valid_purchase = $valid->error_display($form_name, 'discount', $discount_error, 'text');
            }
        }
        ...
        if(!empty($extra_charges_name_error)) {
            if(!empty($valid_purchase)) {
                $valid_purchase = $valid_purchase." ".$valid->error_display($form_name, 'extra_charges_name', $extra_charges_name_error, 'text');
            }
            else {
                $valid_purchase = $valid->error_display($form_name, 'extra_charges_name', $extra_charges_name_error, 'text');
            }
        }
        if(!empty($extra_charges)) {
        ...
        if(!empty($extra_charges_error)) {
            if(!empty($valid_purchase)) {
                $valid_purchase = $valid_purchase." ".$valid->error_display($form_name, 'extra_charges', $extra_charges_error, 'text');
            }
            else {
                $valid_purchase = $valid->error_display($form_name, 'extra_charges', $extra_charges_error, 'text');
            }
        }
        $extra_charges_total = $total_amount;

        $greater_tax = ""; $str_tax = ""; $extra_charges_tax = 0; $product_tax_value = array(); $stock_unique_ids = array();
        if($gst_option == '1' && empty($purchase_error) && empty($valid_purchase)) {
            ...
            if($tax_type =='1'){
                ...
                if(!empty($overall_tax)) {
                    ...
                    if(preg_match("/^[0-9]+(\\.[0-9]+)?$/", $tax)) {
                        ...
                        $purchase_error = "Invalid Overall tax";
                    }
                    if(!empty($product_ids) && empty($purchase_error) && empty($valid_purchase)) {
                        for($j=0; $j < count($product_ids); $j++) {
                            $discounted_product_amount = 0; $tax_value = 0;
                            if(!empty($product_amount[$j])) {
                                $discounted_product_amount = $product_amount[$j];
        ...
        if(empty($purchase_error) && empty($valid_purchase) && $total_amount <= '0') {
            $purchase_error = "Bill value cannot be 0";
        }
            if(!empty($edit_id) && empty($purchase_error) && empty($valid_purchase)) {
        ...
        if(empty($purchase_error) && empty($valid_purchase)){
            ...
            if(preg_match("/^\d+$/", $check_user_id_ip_address)) {
                ...
                else
                {
                    ...
                    $purchase_update_id = $obj->UpdateSQL($GLOBALS['purchase_table'], $getUniqueID, $columns, $values, $action);
                    if(preg_match("/^\d+$/", $purchase_update_id)) {
                        $update_stock = 1; $balance = 1;
                        $purchase_id = $edit_id;
                        $purchase_number = $obj->getTableColumnValue($GLOBALS['purchase_table'], 'purchase_id', $purchase_id, 'purchase_number');
                        $result = array('number' => '1', 'msg' => 'Updated Successfully','redirection_page' =>'purchase.php');
                    }
                    else {
                        $result = array('number' => '2', 'msg' => $purchase_update_id);
                    }	
                }
                // if($update_stock == '1' && !empty($purchase_id) && !empty($purchase_number)) {
                //     $remarks = $purchase_number;
                ...
                //     if(!empty($product_ids)) {
                //         for($i=0; $i < count($product_ids); $i++) {
                //             $stock_update = $obj->StockUpdate($GLOBALS['purchase_table'], 'Out', $purchase_id, $purchase_number, $product_ids[$i], $unit_ids[$i],$size_ids[$i], $remarks, $purchase_date, $quantity[$i],$party_id); 
                //         }
                //     }
                // }
                if(!empty($balance) && $balance == 1 ) {
                    if(!empty($party_id)) {
                        $credit  = 0; $debit = 0; $bill_type ="Purchase";
                        $bill_id = $purchase_id;
            ...
            if(!empty($valid_purchase)) {
                ...
                $result = array('number' => '2', 'msg' => $purchase_error);
            }
        }
        if(!empty($result)) {
    ...
    if(isset($_POST['draw'])){
        ...
        $totalRecords = 0;
        $totalRecords = count($obj->getPurchaseRecordList($row, $rowperpage, $searchValue, $filter_from_date, $filter_to_date, $filter_party_id, $cancelled, $order_column, $order_direction));
        $filteredRecords = count($obj->getPurchaseRecordList('', '', $searchValue, $filter_from_date, $filter_to_date, $filter_party_id, $cancelled, $order_column, $order_direction));

        $data = [];
        
        $purchaseList = $obj->getPurchaseRecordList($row, $rowperpage, $searchValue, $filter_from_date, $filter_to_date, $filter_party_id, $cancelled, $order_column, $order_direction);
        $sno = $row + 1;
        foreach ($purchaseList as $val) {
            $created_date_time = $by_creator_name = ""; $updated_date_time = ""; $purchase_date = ""; $bill_number = ""; $party_name = "";$party_mobile_number =  $bill_total = 0;$purchase_number =""; $party_name_mobile_city ='';$product_count =0 ;
            ...
            if(!empty($val['purchase_date']) && $val['purchase_date'] != "0000-00-00") {
            ...
            if(!empty($val['purchase_number']) && $val['purchase_number'] != $GLOBALS['null_value']) {
            ...
            $edit_access_error = ""; $permission_module = $GLOBALS['purchase_module']; $permission_action = $edit_action;
            if(!empty($login_role_id)) {
                include("user_permission_action.php");
            }
            $delete_access_error = ""; $permission_module = $GLOBALS['purchase_module']; $permission_action = $delete_action;
            if(!empty($login_role_id)) {
                include("user_permission_action.php");
            }
            if(empty($edit_access_error) && empty($val['cancelled'])) {
                $edit_option = '<li><a class="dropdown-item" href="Javascript:ShowModalContent('.'\''.$page_title.'\''.', '.'\''.$val['purchase_id'].'\''.');"><i class="fa fa-pencil"></i>&nbsp; Edit</a></li>';
            }
            if(empty($delete_access_error) && empty($val['cancelled'])) {
                $delete_option = '<li><a class="dropdown-item" href="Javascript:DeleteModalContent('.'\''.$page_title.'\''.', '.'\''.$val['purchase_id'].'\''.');"><i class="fa fa-ban"></i>&nbsp; Cancel</a></li>';
            }
            $conversion_option = "";
            $print_option = '<li><a class="dropdown-item" target="_blank" href="reports/rpt_purchase_a4.php?view_purchase_id=' . $val['purchase_id'] . '"><i class="fa fa-print"></i>&nbsp; A4 Print</a></li>';

            // if(empty($edit_access_error) && empty($val['cancelled'])) {
            //     $conversion_option = '<li><a class="dropdown-item" href="Javascript:ShowBillConversion('.'\''.$page_title.'\''.', '.'\''.$val['purchase_id'].'\''.');"><i class="bi bi-arrow-left-right"></i>&nbsp; Conversion</a></li>';
            // }
            $action = '<div class="dropdown">
                            <a href="#" role="button" class="btn btn-dark py-1 px-2" id="dropdownMenuLink'.$val['purchase_id'].'" data-bs-toggle="dropdown" aria-expanded="false">
                                <i class="bi bi-three-dots-vertical"></i>
                            </a>
                            <ul class="dropdown-menu" aria-labelledby="dropdownMenuLink'.$val['purchase_id'].'">
    ...
    if(isset($_REQUEST['delete_purchase_id'])) {
        $delete_purchase_id = trim(filter_input(INPUT_GET, 'delete_purchase_id', FILTER_SANITIZE_SPECIAL_CHARS));
        $msg = "";
        if(!empty($delete_purchase_id)) {	
            $purchase_unique_id = "";
            $purchase_unique_id = $obj->getTableColumnValue($GLOBALS['purchase_table'], 'purchase_id', $delete_purchase_id, 'id');
            $bill_number = "";
            $bill_number = $obj->getTableColumnValue($GLOBALS['purchase_table'], 'purchase_id', $delete_purchase_id, 'bill_number');
        
            $action = "";
            if(!empty($bill_number)) {
                $action = "purchase Cancelled. Bill No. - ".$bill_number;
            }
            if(empty($delete_access_error)) {
                $delete_id =  $obj->DeletePayment($delete_purchase_id);
                $columns = array(); $values = array();
                $columns = array('cancelled');
                $values = array(1);
                $msg = $obj->UpdateSQL($GLOBALS['purchase_table'], $purchase_unique_id, $columns, $values, $action);
            }
            else {
